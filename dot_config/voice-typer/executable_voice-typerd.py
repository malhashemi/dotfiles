...
def notify(summary: str, body: str = "", urgency: str = "normal") -> None:
    ...
    notify-send (libnotify/swaync); no-ops if the tool is missing."""
...
def paste(cfg: dict) -> None:
    """Trigger the paste shortcut so the focused app inserts the clipboard
    (smart-clipboard.sh -> ALT+V in terminals / CTRL+V in GUI). Pasting inserts
    the text wholesale, sidestepping wtype's per-key capital issues."""
...
class App:
    ...
    def _start_audio_dump(self) -> None:
        ...
        `record_audio` command from config (pw-record)."""
