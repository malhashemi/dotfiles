...
<skill id="code-verify" name="Code Verify" version="1.0">

  <overview>
    <purpose>
      Run one V-phase gate: dispatch codesmith-validator with the
      gate-appropriate angles; render findings into the verification
      doc; return verdict + loop-back target.
    </purpose>

    <inputs>
      Required (all gates):
      - gate ∈ {D-review, P-review, I-per-slice, final-integration}
      - bundle_dir, ticket_path, ticket_alias, ticket_slug, primary_tag
      - tags (full tag array for conditional angle triggering)

      Gate-specific:
      - D-review:           design_path
      - P-review:           plan_paths (list)
      - I-per-slice:        slice_id, commit_sha, worktree_path, plan_path
      - final-integration:  worktree_path, plan_paths (list)
    </inputs>
    ...
    <outputs>
      Returns to caller:
      {
        ...
        "findings_summary": { critical, high, medium, low, suggestion },
        "verification_doc": "/.../verification-YYYY-MM-DD-{slug}.md",
        "user_waivers_applied": <n>  // Medium findings waived in this gate
      }

      Side effects:
      - verification.md created on first call (any gate); appended on
        subsequent calls
      - For BLOCKED Medium findings: user is prompted per finding;
        waivers recorded in the finding's user_waived field
    </outputs>
  </overview>

  <workflow>

    <step n="1" title="Compose Angle Set">
      <action>Load and execute: references/steps/step-01-compose-angles.xml</action>
    </step>

    <step n="2" title="Dispatch Validator">
      <action>Load and execute: references/steps/step-02-dispatch-validator.xml</action>
    </step>

    <step n="3" title="Handle Medium Findings (User Waivers)">
      <action>Load and execute: references/steps/step-03-handle-medium-findings.xml</action>
    </step>

    <step n="4" title="Render Verification Doc Entry">
      <action>Load and execute: references/steps/step-04-render-doc.xml</action>
    </step>

    <step n="5" title="Return Verdict to Caller">
      <action>Load and execute: references/steps/step-05-return-verdict.xml</action>
    </step>
  ...
  <completion>
    <action>
      Returns to the caller (codesmith primary for D/P-review;
      code-implement for I-per-slice / final-integration). Caller acts
      on the verdict + loop-back target per its own routing logic.
    </action>
  </completion>
