...
<skill id="local-merge" name="Local Merge" version="1.0">

  <overview>
    <purpose>
      Squash + rebase + merge + cleanup the implementation worktree into
      the target branch via `wt merge`. Used at V-completion when no PR
      cycle applies.
    </purpose>

    <inputs>
      - branch          The implementation branch to merge (from worktree-setup)
      - target_branch   The target to merge into (typically main / dev / trunk)
    ...
    <preconditions>
      Before invocation:
      - All V-phase gates have returned PROCEED
      - Final-integration gate has passed
      - The branch is in a committed, pushable state (no uncommitted work)
      - PR mode does NOT apply (else invoke pr-cycle instead)
    </preconditions>
  </overview>
  ...
  <workflow>
    ...
    <step n="1" title="Verify Preconditions">
      <action>Load and execute: references/steps/step-01-verify-preconditions.xml</action>
    </step>

    <step n="2" title="Run wt merge">
      <action>Load and execute: references/steps/step-02-run-wt-merge.xml</action>
    </step>

    <step n="3" title="Verify Post-Merge State">
      <action>Load and execute: references/steps/step-03-verify-post-merge.xml</action>
    </step>

    <step n="4" title="Return Handoff">
      <action>Load and execute: references/steps/step-04-return-handoff.xml</action>
    </step>
  ...
  <completion>
    <action>
      Code is on the target branch. Worktree is removed. Caller
      (code-verify or code-implement final stage) transitions the parent
      ticket from `implemented` to `verified` via
      transition-ticket-status.py.
    </action>
  </completion>
