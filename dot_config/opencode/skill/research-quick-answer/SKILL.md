...
<skill id="research-quick-answer" name="Research Quick Answer" version="1.0">

  <overview>
    <purpose>
      Answer one or more focused factual questions and return the answers
      inline with citations. Dispatch the right research child per
      question shape; synthesize per question; never persist a research
      document — the fast path for lookups, freshness checks, and
      grilling-loop research batches.
    </purpose>

    <inputs>
      Interactive (user invokes [QA]):
      - One or more questions in natural language; scope and output shape
        elicited if the question is too vague to dispatch.

      Autonomous (dispatched from a workflow):
      - questions: array of entries, each with
        - question_text (required)
        - scope (optional; directory globs, file paths, or domain)
        - child_subagent_hint (optional; overrides shape inference)
        - expected_output_shape (required)
      - web_search_approved (boolean; required true for any question
        that routes to web-search-researcher)
    </inputs>

    <outputs>
      Inline synthesis only — no persisted document. Per question:
      answer with citations, confidence level, and gap note. Autonomous
      mode returns a structured signal (SUCCESS / ERROR /
      CLARIFICATION_NEEDED).
    </outputs>

    <key-discipline>
      Hide-the-intent: every child prompt carries the question alone —
      never the parent task's intent.

      Silence is data: "searched X, Y, Z; no matches" is a finding, not
      a failure. A negative finding ships as a cited answer.

      No persistence: if a lookup turns out to be load-bearing, the
      caller escalates to [FS] Focused Study or [CI] Comprehensive
      Investigation, which persist.
    </key-discipline>
  </overview>
  ...
  <workflow>
    ...
    <step n="2" title="Plan Dispatch">
      <action>Load and execute: references/steps/step-02-plan-dispatch.xml</action>
    </step>

    <step n="3" title="Dispatch and Synthesize">
      <action>Load and execute: references/steps/step-03-dispatch-synthesize.xml</action>
    </step>

    <step n="4" title="Return">
      <action>Load and execute: references/steps/step-04-return.xml</action>
    </step>
  ...
  <completion>
    <action>
      After step 4 returns the synthesis, the lookup is complete. In
      interactive mode, offer a follow-up question or escalation to a
      persisting capability. In autonomous mode, the structured signal
      is the final return to the dispatching caller.
    </action>
  </completion>
