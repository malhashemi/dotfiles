...
from utils import hex_to_argb, get_material_colors, get_catppuccin_colors, is_dynamic_theme, opacity_to_alpha
...
class SketchybarTheme(BaseApp):
    ...
    def generate_opacity(self, theme_data: dict) -> None:
        ...
        self.log_progress(f"Generating Sketchybar opacity ({opacity_percent}%)", emoji="🔲")
...
# Usage: add_opacity "0xff1e1e2e" → "0xa51e1e2e" (with current opacity)
add_opacity() {{
  local color="$1"
...
"""
    ...
    def reload(self) -> None:
        ...
        success = self.run_command(
            ["sketchybar", "--reload"],
            error_msg="Sketchybar reload failed"
        )
